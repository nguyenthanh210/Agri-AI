...
@router.post("/google-login", response_model=TokenDTO)
async def google_login(
    token_data: GoogleLoginDTO,
    repository: SQLAlchemyUserRepository = Depends(get_user_repository),
):
    """
    Login / register via Google ID token.
    Frontend sends the idToken obtained from google_sign_in.
    """
    use_case = GoogleLoginUseCase(repository)
    try:
        return await use_case.execute(token_data)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=str(e),
